...
| A-10 | 已合入 main | feat/admin-ops | — | 运营策略配置（匹配窗口 / 金额容差 / 备份过期）；生产 SQL 已执行 |
...
- 2026-08-31：生产已执行 202608310012_admin_platform_limits.sql、202608310013_feedback_intent_submit_limit.sql。限额配置页拆出运营策略区块（匹配日期接近天数 0–30、匹配金额容差 0–100 分、备份过期天数 1–90）。占用改为账户页权益展示 / 账龄分桶边界配置 / 安全空闲。
