...
U-19 账号资料与头像：账户页去掉「最近使用」；可上传正方形头像、填写称呼 / 公司 / 职务 / 城市 / 简介。资料写入 profiles，头像裁成 256 边长后保存在档案里。不把图片写进登录令牌。
...
| U-19 | 已合入 main | feat/user-workspace | ab3ed66 | 账号资料与头像 |
...
A-14 留言档案回访与风控补齐：留言写入提交账号并可点进档案；意向「已发试用」真正发放试用（已有生效权益则跳过）；工作台「今日该回访」；关闭全员限免前展示将失去入口的人数与邮箱；档案补资料/意向/留言/登录冷却（不含头像）；风控台登录冷却列表可解锁；公告可预约开始并显示不再弹出人数。不接支付、不假入账。
A-15 档案停用提权、留言备注、账号回填：用户档案可停用/提权；最后一个可用管理员不能被取消或停用；留言可写内部备注并认领处理人；意向/留言按邮箱回填注册账号（含新注册触发）；工作台搜邮箱打开档案；风控台列出今日新地点登录。
A-16 意向认领检索与风控名单：购买意向可认领处理人；留言与意向支持关键字检索；工作台和风控台列出未验证账号、未开二次验证的管理员、云函数今日调用；档案里的意向/留言可跳到对应列表并高亮。不接支付、不发信、不批量发试用。
...
| A-08 | 预埋 / 待支付 | feat/admin-ops | 2be63bf | 邀请表与后台列表；开关默认关；不入账 |
...
| A-14 | 已合入 main | feat/admin-ops | 7e3e4a8 | 留言档案回访与风控补齐；92 tests；build PASS；生产 SQL 已执行；已部署 submit-feedback |
| A-15 | 已合入 main | feat/admin-ops | 2be63bf | 档案停用提权、留言备注、账号回填、工作台搜邮箱、新地点登录；93 tests；build PASS；生产 SQL 已执行 |
| A-16 | 已合入 main | feat/admin-ops | 4117ee6 | 意向认领、关键字检索、未验证/未开二次验证/云函数用量；94 tests；build PASS；生产 SQL 已执行 |
...
- 2026-09-03：合入 main：真实使用小坑（126e819）。银行导出可跳过标题行；识别 20260831、中文日期和日/月/年；重复行说明原因；手机上月结和应收改为卡片叠放。占用改为五个财税工具打磨 / 空闲 / 安全空闲。
...
- 2026-09-02：合入 main：留言读取权限（ba46a03）。管理员读留言改走接口；表权限与提交通道对齐。占用改为五个财税工具打磨 / 空闲 / 安全空闲。
- 2026-09-02：合入 main：账号资料与头像（ab3ed66）。去掉最近使用；可上传头像并填写公司 / 职务 / 城市 / 简介。占用改为五个财税工具打磨 / 空闲 / 安全空闲。
- 2026-09-02：合入 main：二次验证开启失败（30a7218）。列举验证器改走当前账号接口；二维码 SVG 转成图片地址。占用改为五个财税工具打磨 / 空闲 / 安全空闲。
- 2026-09-02：合入 main：试用到期预警与用户详情（ce25533）。工作台列出预警天数内到期的单独授予；用户详情含二次验证、会话、到期日、登录地点，可续发/收回/下线。生产已执行 202609020020_admin_dossier_expiry.sql。占用改为五个财税工具打磨 / 空闲 / 安全空闲。
