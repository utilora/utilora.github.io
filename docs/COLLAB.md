...
| A-11 | 已合入 main | feat/admin-ops | 48e4294 | 账龄分桶边界配置；非法序拒绝；预览五桶；用户端读配置；82 tests；build PASS |
...
- 2026-08-31：合入 main：账龄分桶边界配置（48e4294）。限额页独立账龄组与五桶预览；非法序拒绝保存并写审计；get_aging_bucket_bounds 供新打开的账龄视图读取。占用改为五个财税工具打磨 / 空闲 / 安全空闲。
